...
async def create_patient(
    patient: PatientCreate, current_user: TokenData = Depends(get_current_clinic_user)
):
    db = get_db()
    # Check if patient exists
    existing = await db.patients.find_one(
        {"phone": patient.phone, "clinic_id": current_user.clinic_id}
    )
    if existing:
        raise HTTPException(
    ...
    result = await db.patients.insert_one(
        patient_db.model_dump(by_alias=True, exclude=["id"])
    )
...
async def get_patient(
    ...
    db = get_db()
    item = await db.patients.find_one(
...
async def get_patient_profile(
    ...
    visits = (
        await db.visits.find({"patient_id": id, "clinic_id": current_user.clinic_id})
